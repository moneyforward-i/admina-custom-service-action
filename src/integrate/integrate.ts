...
export const Sync = async (src: string, dist: string, env: NodeJS.ProcessEnv) => {
...
const syncToAdmina = async (source: Source, env: NodeJS.ProcessEnv) => {
        switch (source) {
                case Source.AzureAd:
                        ...
                        await Promise.all(azureAdData.map(async (app: AzureAdSource.AppInfo) => {
                                AdminaDist.registerCustomService(await AzureAdTransform.transformDataToAdmina(app), env);
                        }));
                        break;
                default:
