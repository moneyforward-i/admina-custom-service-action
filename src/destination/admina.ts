...
export function registerCustomService(app: AppInfo, env: NodeJS.ProcessEnv): void {
...
class Admina {
        ...
        public async registerCustomService(appInfo: AppInfo): Promise<void> {
                ...
                const serviceListResponse = await axios.get(serviceListEndpoint, this.request_header);
                ...
                if (!workspaceId) {
                        ...
                        const customServiceResponse = await axios.post(customWsEndpoint, customWsPayload, customWsConfig);
                ...
                await this.registerUserAccountToCustomWorkspace(serviceId, workspaceId, targetWorkspaceName, appInfo.users);
        }

        private async registerUserAccountToCustomWorkspace(serviceId: number, workspaceId: number, wsName: string, users: UserInfo[]): Promise<void> {
                ...
                const accountListResponse = await axios.get(accountListEndpoint, this.request_header)
                const accountEmails = accountListResponse.data.items.map((account: any) => account.email)

                // Obtain a list of accounts for each creation, renewal, and deletion
                const existingUsers = users.filter((user: UserInfo) => accountEmails.includes(user.email));
                const newUsers = users.filter((user: UserInfo) => !accountEmails.includes(user.email));
                const deletedUsers: { email: string, displayName: string }[] = accountListResponse.data.items.filter((account: any) => !users.find((user: UserInfo) => user.email === account.email));
                ...
                try {
                        ...
                        console.error(`Error occurred while registering user account into [${wsName}]:`, error.message, error.response.data);
                }
